...
export function ArticleFooter({ articleSlug, showCta = true }: ArticleFooterProps) {
  ...
  return (
    <section className="mt-10 border-t border-stone-200 pt-8">
            ...
            {articleLinks.map((link) => {
              ...
              return (
                ...
                >
                  {target ? (
                    <>
                      <span className="rounded-md border border-stone-200 bg-stone-50 px-2 py-0.5 text-[10px] font-medium text-stone-500">
                        {CATEGORY_LABELS[target.category]}
                      </span>
                      <span className="mt-2 block text-sm font-medium text-stone-900 leading-snug line-clamp-2">
                        {target.title}
                      </span>
      ...
      <div className="ds-section-gap flex flex-col gap-2">
        <TrackedLink
          href={categoryAnchor}
          className="text-sm font-medium text-stone-700 hover:text-emerald-900 hover:underline"
          event={{
