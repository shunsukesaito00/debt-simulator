#!/usr/bin/env node
/**
 * 計画に基づくシンプル記事ページを一括生成（初回のみ実行想定）
 * Usage: node scripts/generate-plan-articles.mjs
 */
...
for (const slug of SLUGS) {
  const dir = join(root, "app", "articles", slug);
  ...
  writeFileSync(file, pageSource(slug), "utf-8");
